#!/usr/bin/env node
/**
 * Kopiert noteQuartal → note für alle Schüler Jg. 8–10
 * im aktuellen (oder angegebenen) Schuljahresabschnitt.
 *
 * Verwendung:
 *   node scripts/copy-quartal-zu-halbjahr.mjs \
...
function parseArgs() {
  ...
  if (!url || !schema || !user || pass === null) {
    console.error(
      'Verwendung: node copy-quartal-zu-halbjahr.mjs\n' +
...
async function main() {
  ...
  let kopiert = 0, ohnequartal = 0, fehler = 0
  ...
  for (const s of schueler) {
    ...
    for (const ld of la.leistungsdaten) {
      if (!ld.noteQuartal) { ohnequartal++; continue }
      if (!args.dryRun) {
        try {
          const body = Object.fromEntries(
            Object.entries({ ...ld, note: ld.noteQuartal }).filter(([, v]) => v !== null)
          )
        ...
        console.log(`  [dry] ${name}: FachID ${ld.fachID} ${ld.noteQuartal} → note`)
        kopiert++
      }
    }
  }
  ...
  console.log(`\n\nErgebnis:`)
  console.log(`  ✓ ${kopiert} Noten kopiert (Quartal → Halbjahr)`)
  console.log(`  – ${ohnequartal} Einträge ohne Quartalsnote übersprungen`)
